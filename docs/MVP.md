...
Shipped: cultures, routes, markers, provinces, religions, named features, landform templates with editable heightmap recipes (hill / pit / range / strait), click-to-stamp blobs, click-to-link roads, paint and inspector edits for every thematic layer, diplomacy stance editor, population layer, folio PNG export, example worlds, layer presets, independent society re-roll, search roster, measure, redo, help, autosave restore (IndexedDB + slots), worker generation, notes, label collision, status bar, neighbor diplomacy in the inspector. Every generated layer can be edited without a full regenerate: paint, inspector selects, names (including rivers), stances, and recipe-apply-to-current-map.
